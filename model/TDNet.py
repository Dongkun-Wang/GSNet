...
class TDNet(nn.Module):
    def __init__(self,seq_len, pre_len, num_of_nodes,num_of_input_feature,
        ...
        self.temporal_model = GSformer(input_dim=num_of_nodes, output_dim=self.north_south_map*self.west_east_map,
                                       embed_dim=64, num_heads=4, num_layers=3, feedforward_dim=128, seq_len=st_embedding_dim, tau=0.1, dropout=0.1)


        


        self.output_layer = nn.Linear(st_embedding_dim*north_south_map*west_east_map,pre_len*north_south_map*west_east_map)
    ...
    def forward(self,grid_input,target_time_feature,graph_feature,
        ...
        final_output = self.output_layer(temporal_output).view(batch_size,-1,self.north_south_map,self.west_east_map)
        return final_output
